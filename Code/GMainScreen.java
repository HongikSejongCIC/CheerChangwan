	...
	GMainScreen(){
		...
		조회.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				if(DataBase.board.isSelectionEmpty()&&DataBase.best.isSelectionEmpty()&&DataBase.Gong.isSelectionEmpty()){
					...
					GMainScreen.bg.setVisible(false);
					GSeeScreen ss = new GSeeScreen(s);
				}
		...
		등록.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				GMainScreen.bg.setVisible(false);
				GRegisterScreen Grs = new GRegisterScreen();	
			}
		});
		등록.setSize(60,50);
		...
		수정.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				if(DataBase.Gong.isSelectionEmpty()){
					JOptionPane.showMessageDialog(null,"선택된 글이 없습니다.");
				}
				else{
					String resultStr = null;
					resultStr = JOptionPane.showInputDialog("비밀 번호를 입력하세요.");
					if(resultStr.equals(DataBase.manager.get(DataBase.Loginid))){
						int result = JOptionPane.showConfirmDialog(null, "수정하시겠습니까?","Change",
						JOptionPane.YES_NO_OPTION,JOptionPane.WARNING_MESSAGE);
					if(result == 0){
						GMainScreen.bg.setVisible(false);
						GChangeScreen Grs = new GChangeScreen((String) DataBase.board.getSelectedValue());	
					}
				}
					else{
		...
		삭제.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				if(DataBase.board.isSelectionEmpty()&&DataBase.Gong.isSelectionEmpty()){
					JOptionPane.showMessageDialog(null,"선택된 글이 없습니다.");
				}
				else if(DataBase.board.isSelectedIndex(DataBase.board.getSelectedIndex())){
					String resultStr = null;
					...
					if(result == 0){
						...
						if(DataBase.listModel3.getSize()==0){ 
						    삭제.setEnabled(false);
						   }
